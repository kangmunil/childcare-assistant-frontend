      ...
      <header className="flex justify-between items-center mb-4 px-1 shrink-0 relative z-40">
        ...
        <div className="flex gap-2 flex-wrap items-center">
            {children.map(child => (
                ...
                >
                    <div className="w-4 h-4 rounded-full bg-white/30 overflow-hidden flex items-center justify-center">
                        <span className="text-[8px] font-bold leading-none">{child.name?.charAt(0)}</span>
                    </div>
                    {child.name}
                </button>
            ))}
